...
* **Designed** an end-to-end RAG ingestion pipeline that pulls PDF/DOCX CV attachments from ClickUp via REST API, extracts and normalizes raw text, then uses an LLM with structured prompting to parse each resume into a consistent schema — name, skills, experience, and summary — ready for indexing.
